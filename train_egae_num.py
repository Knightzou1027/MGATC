...
def parse_args():
    parser = argparse.ArgumentParser(description='Multi-View Deep Graph Clustering for Trip Purpose Inference (EGAE variant without categorical features)')

    # 数据参数
    parser.add_argument('--feature_data_path', type=str, default='./data/od_idv_train_ptype0_gz_1018.xlsx',
    ...
    parser.add_argument('--n_clusters', type=int, default=15, help='Fixed number of clusters for trip purpose inference.')
...
def main():
    ...
    try:
        ...
        # 检查所有需要的列是否存在
        missing_in_file = [col for col in NUMERICAL_FEATURE_COLUMN_NAMES if col not in df_features_full.columns]
        if missing_in_file:
    ...
    #预训练结果保存
    pretrain_results_csv_filename = f"n{args.n_clusters}_pretrain_standard_kmeans_clustering_gz_ptype0_no_cat.csv"
    ...
    # 单独保存预训练评估的详细指标到一个文件
    pretrain_eval_metrics_filename = f"n{args.n_clusters}_pretrain_standard_kmeans_evaluation_metrics_gz_ptype0_no_cat.txt"
    ...
    # 保存预训练模型 (只保存 encoder)
    pretrain_encoder_filename = f"n{args.n_clusters}_pretrain_encoder_gz_pytpe0_no_cat.pth"
    ...
    final_model_filename = f"n{args.n_clusters}_final_model_gz_ptype0_no_cat.pth"
    ...
    #保存最终结果
    final_results_csv_filename = f"n{args.n_clusters}_final_clustering_gz_pytpe0_no_cat.csv"
    ...
    # 单独保存最终评估的详细指标到一个文件
    final_eval_metrics_filename = f"n{args.n_clusters}_final_evaluation_metrics_gz_ptype0_no_cat.txt"
